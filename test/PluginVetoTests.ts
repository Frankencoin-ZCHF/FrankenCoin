...
describe("Plugin Veto Tests", () => {
    ...
    before(async () => {
        ...
        mintingHubContract = await createContract("MintingHub", [zchf.address, positionFactoryContract.address]);
        // mocktoken
    ...
    describe("create secondary bridge plugin", () => {
        it("create mock DCHF token&bridge", async () => {
            ...
            secondBridge = await createContract("StablecoinBridge", [mockDCHF.address, zchf.address, limit]);
        });
        it("Participant suggests minter", async () => {
            ...
            await expect(zchf.connect(alice).suggestMinter(secondBridge.address, applicationPeriod,
                applicationFee, msg)).to.emit(zchf, "MinterApplied");
        });
        it("can't mint before min period", async () => {
            ...
            // set allowance
            await expect(secondBridge.connect(alice).mint(amount)).to.be.revertedWithCustomError(zchf, "NotMinter");
        });
        it("deny minter", async () => {
            await expect(zchf.denyMinter(secondBridge.address, [], "other denied")).
                to.emit(zchf, "MinterDenied");
            await expect(secondBridge.connect(alice).mint(floatToDec18(1_000))).
