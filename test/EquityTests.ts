...
describe("Equity Tests", () => {
    ...
    describe("minting shares", () => {
        ...
        it("should refuse redemption before time passed", async () => {
            ...
            await expect(equity.redeem(owner.address, floatToDec18(0.1))).to.be.revertedWithoutReason();
        });
        it("should allow redemption after time passed", async () => {
            ...
            let redemptionAmount = (await equity.balanceOf(owner.address)).sub(floatToDec18(1000.0));
    ...
    describe("transfer shares", () => {
        it("total votes==sum of owner votes", async () => {
            ...
            let votesBefore = [await equity.votes(owner.address), await equity.votes(other)];
        ...
        it("total votes correct after transfer", async () => {
            ...
            let totVotesBefore = await equity.totalVotes();
            let votesBefore = [await equity.votes(owner.address), await equity.votes(other)];
            let balancesBefore = [await equity.balanceOf(owner.address), await equity.balanceOf(other)];
            await equity.transfer(other, floatToDec18(amount));
            let balancesAfter = [await equity.balanceOf(owner.address), await equity.balanceOf(other)];
            expect(balancesAfter[1] > balancesBefore[1]).to.be.true;
            let totVotesAfter = await equity.totalVotes();
            let votesAfter = [await equity.votes(owner.address), await equity.votes(other)];
        ...
        it("delegate vote", async () => {
            ...
            let qualified1 = await equity.votesDelegated(owner.address, [bob.address, alice.address]);
            let qualified2 = await equity.votesDelegated(owner.address, []);
            expect(qualified1 > qualified2).to.be.true;
            await expect(equity.votesDelegated(bob.address, [alice.address])).to.be.reverted;
        });
