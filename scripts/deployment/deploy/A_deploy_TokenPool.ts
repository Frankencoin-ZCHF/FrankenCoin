...
const deploy: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const txOwnerTransfer = await burnMintTokenPool.transferOwnership(await ccipAdminContract.getAddress())
  ...
  const tx = await ccipAdminContract.setTokenPool(await burnMintTokenPool.getAddress());
  await tx.wait()
  
  console.log("Add pool as minter")
  const txMinter = await zchfContract.initialize(await burnMintTokenPool.getAddress(), "CCIP Token Pool")
  await txMinter.wait()
};

export default deploy;
deploy.tags = ["main", "TokenPool"];
