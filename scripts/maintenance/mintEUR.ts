...
async function mintXEUR(amount: number, address: string) {
    const wallet = new ethers.Wallet(pk);

    let tokenContract = await getSigningManagerFromPK(mockXEURAddr, ERC20_ABI, NODE_URL, pk);
...
async function mintdEURO(amount: number) {
    // mint dEURO via XEUR bridge
    ...
    console.log("minted dEURO:", tx.hash);
}
...
async function start() {
    ...
    //await mintXEUR(amount, address);
    await mintdEURO(amount);
}
